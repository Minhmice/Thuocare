...
- **MVP Scope:** For the MVP, this "Forgot Password" screen will primarily serve as a placeholder. It will contain clear, short copy guiding the user on the next steps (e.g., "Please contact support to reset your password" or "Password reset functionality is coming soon."). It will _not_ initiate an actual password reset flow (e.g., email verification, SMS OTP), as the full recovery flow is deferred. This interaction will be a full-screen transition, not a modal on the Sign In screen itself, to simplify the flow and reduce cognitive load.
