...
- Invert the surface logic: elevated layers become slightly _lighter_ shades of the base color to maintain the nested depth without blinding the user.
...
- **Text Overflow & Truncation:** Medical terms can be long (e.g., _Acetaminophen & Codeine Phosphate_). All card titles must enforce a 2-line maximum (`line-clamp: 2`) with an ellipsis (`...`) to prevent the card structure from breaking.
...
- **Focus States (Accessibility Fix):** To ensure visually impaired users or those experiencing cognitive fatigue can see active inputs, focus rings must use `outline` at **min 40% opacity** of the primary color, accompanied by a subtle background tint shift. _Never rely on sub-20% ghost borders for interaction states._
...
- **Strict Negative Space Enforcement:** If content exceeds the screen height, _do not shrink padding to make it fit_. Always utilize vertical scrolling. Protect the negative space at all costs.
